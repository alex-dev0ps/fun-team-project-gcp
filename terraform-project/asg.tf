...
resource "google_compute_autoscaler" "team3" {
     depends_on = [
        google_sql_database_instance.mysql-from-terraform,
...
resource "google_compute_instance_template" "compute-engine" {
  ...
  metadata_startup_script = <<SCRIPT
    ...
    sed 's/'localhost'/'${google_sql_database_instance.mysql-from-terraform.ip_address.0.ip_address}'/g' /var/www/html/wp-config.php -i
